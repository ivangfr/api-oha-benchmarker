...
public class OhaProcessor {
  ...
  private static final String OHA_COMMAND =
      "oha -n %s -c %s -u s --latency-correction --disable-keepalive --disable-color --no-tui http://localhost:%s/%s";

  public List<Double> run(
      int numRequests, int concurrency, int containerMappedPort, String endpoint) {
    List<Double> ohaMetrics;
    try {
      String ohaCommand =
          OHA_COMMAND.formatted(numRequests, concurrency, containerMappedPort, endpoint);
  ...
  private static final String[] LINES_TO_PARSE =
      new String[] {"Success rate:", "Total:", "Slowest:", "Fastest:", "Average:", "Requests/sec:"};
}
